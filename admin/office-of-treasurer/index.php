...
require_access('office-of-treasurer');

$page_title = 'Office of the Treasurer';
...
$tr_name      = $s['tr_name'] ?? 'Not configured';
$tr_title     = $s['tr_title'] ?? '';
$pa_name      = $s['pa_name'] ?? '';
$pa_title     = $s['pa_title'] ?? '';
$tr_photo     = $s['tr_photo'] ?? '';
...
<div class="d-flex justify-content-between align-items-center mb-4">
    <nav aria-label="breadcrumb">
        <ol class="breadcrumb mb-0">
            ...
            <li class="breadcrumb-item active">Office of the Treasurer</li>
        </ol>
    </nav>
    <a href="<?= $frontend_base ?>/office-of-treasurer.php" target="_blank"
...
<div class="alert <?= $is_published ? 'alert-success' : 'alert-warning' ?> d-flex align-items-center gap-2 mb-4" style="border-radius:12px;">
    ...
    <span>Page is currently <strong><?= $is_published ? 'Published' : 'Unpublished' ?></strong>.
        <?php if (!$is_published): ?>
        <a href="<?= APP_URL ?>/office-of-treasurer/settings.php" class="alert-link">Enable publishing in settings.</a>
        <?php endif; ?></span>
</div>
            ...
            <div class="card-body p-4">
                <div class="d-flex align-items-start gap-3">
                    ...
                    <div style="width:56px;height:56px;border-radius:50%;background:linear-gradient(135deg,#002147,#1a4faf);display:flex;align-items:center;justify-content:center;flex-shrink:0;">
                        <i class="fas fa-coins" style="color:#fff;font-size:1.3rem;"></i>
                    </div>
                    <?php endif; ?>
                    <div class="flex-grow-1">
                        <h6 class="mb-1 fw-semibold" style="color:#002147;"><i class="fas fa-coins me-2 text-muted" style="font-size:.85rem;"></i>Treasurer</h6>
                        <div class="fw-medium"><?= h($tr_name) ?></div>
                        <div style="font-size:.8rem;color:#64748b;"><?= h($tr_title) ?></div>
                    </div>
                </div>
                <hr style="border-color:#f1f5f9;margin:16px 0 12px;">
                <?php if (can_access('office-of-treasurer', 'can_edit')): ?>
                <a href="<?= APP_URL ?>/office-of-treasurer/settings.php?tab=treasurer" class="btn btn-sm btn-primary" style="border-radius:8px;font-size:.8rem;">
                    <i class="fas fa-edit me-1"></i> Edit Treasurer Profile
                </a>
                <?php endif; ?>
            </div>
        </div>
        ...
        <div class="card h-100" style="border-top:4px solid #1a4faf;border-radius:12px;">
            <div class="card-body p-4">
                <div class="d-flex align-items-start gap-3">
                    ...
                    <div class="flex-grow-1">
                        <h6 class="mb-1 fw-semibold" style="color:#1a4faf;"><i class="fas fa-id-badge me-2 text-muted" style="font-size:.85rem;"></i>PA to Treasurer</h6>
                        <div class="fw-medium"><?= $pa_name !== '' ? h($pa_name) : '<span style="color:#94a3b8;font-style:italic;font-size:.85rem;">Not configured</span>' ?></div>
                        <div style="font-size:.8rem;color:#64748b;"><?= h($pa_title) ?></div>
                    </div>
                </div>
                <hr style="border-color:#f1f5f9;margin:16px 0 12px;">
                <?php if (can_access('office-of-treasurer', 'can_edit')): ?>
                <a href="<?= APP_URL ?>/office-of-treasurer/settings.php?tab=pa" class="btn btn-sm btn-outline-primary" style="border-radius:8px;font-size:.8rem;">
                    <i class="fas fa-edit me-1"></i> Edit PA Profile
                </a>
                <?php endif; ?>
            </div>
        </div>
        ...
        <div class="card h-100" style="border-top:4px solid #D21034;border-radius:12px;">
            <div class="card-body p-4">
                <div class="d-flex align-items-start gap-3">
                    ...
                    <div class="flex-grow-1">
                        <h6 class="mb-1 fw-semibold" style="color:#D21034;">Treasurer's Message</h6>
                        <div style="font-size:.82rem;color:#64748b;"><?= h($s['message_title'] ?? 'Message from the Treasurer') ?></div>
                ...
                <hr style="border-color:#f1f5f9;margin:16px 0 12px;">
                <?php if (can_access('office-of-treasurer', 'can_edit')): ?>
                <a href="<?= APP_URL ?>/office-of-treasurer/settings.php?tab=message" class="btn btn-sm btn-outline-danger" style="border-radius:8px;font-size:.8rem;">
            ...
            <div class="card-body p-4">
                <div class="d-flex align-items-start gap-3">
                    ...
                    <div class="flex-grow-1">
                        ...
                        <div style="font-size:.82rem;color:#64748b;"><?= h($s['hero_title'] ?? 'Office of the Treasurer') ?></div>
                        <div class="mt-2">
                ...
                <hr style="border-color:#f1f5f9;margin:16px 0 12px;">
                <?php if (can_access('office-of-treasurer', 'can_edit')): ?>
                <a href="<?= APP_URL ?>/office-of-treasurer/settings.php?tab=page" class="btn btn-sm btn-outline-success" style="border-radius:8px;font-size:.8rem;">
