...
require_access('email-templates', 'can_create');
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        $dup = db()->prepare('SELECT id FROM email_templates WHERE action = ?');
        $dup->execute([$action]);
        if ($dup->fetch()) $errors[] = 'An email template with this action already exists.';
    }
    ...
    if (empty($errors)) {
        db()->prepare(
            'INSERT INTO email_templates (name, action, subject, body_html, variables, is_active)
             VALUES (?,?,?,?,?,?)'
        )->execute([$name, $action, $subject, $body_html, $variables ?: null, $is_active]);

        flash_set('success', "Email template <strong>" . h($name) . "</strong> created.");
        redirect(APP_URL . '/email-templates/index.php');
    }

    save_old(compact('name', 'action', 'subject', 'variables'));
}
...
<div class="card">
    <div class="card-header py-3 px-4">
        <h6 class="mb-0 fw-semibold"><i class="fas fa-plus me-2 text-muted"></i>New Email Template</h6>
    </div>
            ...
            <div class="row g-3">
                ...
                <div class="col-md-6">
                    ...
                    <input type="text" name="action" class="form-control"
                           value="<?= old('action') ?>" required maxlength="100"
                           placeholder="e.g. forgot_password" autocomplete="off">
                ...
                <div class="col-12">
                    ...
                    <input type="text" name="subject" class="form-control"
                           ...
                           placeholder="e.g. Reset Your Password – {{app_name}}">
                </div>
                <div class="col-12">
                    <label class="form-label fw-medium">Available Variables</label>
                    <input type="text" name="variables" class="form-control"
                           value="<?= old('variables') ?>" maxlength="500"
                           placeholder="e.g. {{full_name}},{{reset_link}},{{app_name}}">
                ...
                <div class="col-12">
                    ...
                    <textarea name="body_html" id="body_html" class="form-control font-monospace"
                              rows="18" required
                              style="font-size:.82rem;resize:vertical;"><?= htmlspecialchars($_POST['body_html'] ?? '', ENT_QUOTES, 'UTF-8') ?></textarea>
