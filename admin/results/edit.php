<?php
require_once __DIR__ . '/../includes/auth.php';
require_access('results', 'can_edit');
require_once __DIR__ . '/helpers.php';

$id   = (int)($_GET['id'] ?? 0);
$exam = rm_get_exam($id);

$page_title = 'Edit Result Exam';
$errors     = [];
clear_old();

$departments = db()->query(
    'SELECT id, name, faculty_label FROM dept_departments WHERE is_active = 1 ORDER BY name ASC'
)->fetchAll();

$semesters = rm_semester_list();

// Programs for current dept
$programs = [];
if ($exam['dept_id']) {
    $stmt = db()->prepare(
        'SELECT id, program_name FROM dept_academic_programs WHERE dept_id = ? AND is_active = 1 ORDER BY program_name ASC'
    );
    $stmt->execute([$exam['dept_id']]);
    $programs = $stmt->fetchAll();
}

if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    csrf_check();

    $dept_id             = (int)($_POST['dept_id'] ?? 0);
    $program_id          = (int)($_POST['program_id'] ?? 0);
    $batch               = trim($_POST['batch'] ?? '');
    $enrollment_semester = trim($_POST['enrollment_semester'] ?? '');
    $completion_semester = trim($_POST['completion_semester'] ?? '');
    $exam_title          = trim($_POST['exam_title'] ?? '');
    $exam_level          = trim($_POST['exam_level'] ?? '');
    $notes               = trim($_POST['notes'] ?? '');
    $is_published        = isset($_POST['is_published']) ? 1 : 0;

    if ($dept_id <= 0)     $errors[] = 'Department is required.';
    if ($exam_title === '') $errors[] = 'Exam Title is required.';

    if ($dept_id > 0) {
        $dv = db()->prepare('SELECT id FROM dept_departments WHERE id = ? AND is_active = 1');
        $dv->execute([$dept_id]);
        if (!$dv->fetch()) $errors[] = 'Invalid department.';
    }

    if (empty($errors) && $program_id > 0) {
        $pv = db()->prepare('SELECT id FROM dept_academic_programs WHERE id = ? AND dept_id = ?');
        $pv->execute([$program_id, $dept_id]);
        if (!$pv->fetch()) $program_id = 0;
    }

    if (empty($errors)) {
        db()->prepare(
            'UPDATE result_exams
             SET dept_id=?, program_id=?, batch=?, enrollment_semester=?,
                 completion_semester=?, exam_title=?, exam_level=?, notes=?, is_published=?
             WHERE id=?'
        )->execute([
            $dept_id,
            $program_id          ?: null,
            $batch               ?: null,
            $enrollment_semester ?: null,
            $completion_semester ?: null,
            $exam_title,
            $exam_level          ?: null,
            $notes               ?: null,
            $is_published,
            $id,
        ]);
        flash_set('success', 'Result exam updated.');
        redirect(APP_URL . '/results/view.php?id=' . $id);
    }

    save_old(compact('dept_id','program_id','batch','enrollment_semester','completion_semester',
                     'exam_title','exam_level','notes','is_published'));
    // Reload programs for the newly selected dept
    if ($dept_id > 0) {
        $stmt2 = db()->prepare(
            'SELECT id, program_name FROM dept_academic_programs WHERE dept_id = ? AND is_active = 1 ORDER BY program_name ASC'
        );
        $stmt2->execute([$dept_id]);
        $programs = $stmt2->fetchAll();
    }
}

// Use old() values for form if present, else exam row
$v_dept    = old('dept_id')    ?: $exam['dept_id'];
$v_prog    = old('program_id') ?: $exam['program_id'];

require_once __DIR__ . '/../includes/header.php';
?>

<div class="d-flex justify-content-between align-items-center mb-4">
    <nav aria-label="breadcrumb">
        <ol class="breadcrumb mb-0">
            <li class="breadcrumb-item"><a href="<?= APP_URL ?>/index.php">Dashboard</a></li>
            <li class="breadcrumb-item"><a href="<?= APP_URL ?>/results/index.php">Results</a></li>
            <li class="breadcrumb-item"><a href="<?= APP_URL ?>/results/view.php?id=<?= $id ?>">
                <?= h($exam['exam_title']) ?></a></li>
            <li class="breadcrumb-item active">Edit</li>
        </ol>
    </nav>
</div>

<?php if ($errors): ?>
<div class="alert alert-danger alert-dismissible fade show">
    <ul class="mb-0 ps-3"><?php foreach ($errors as $e): ?><li><?= h($e) ?></li><?php endforeach; ?></ul>
    <button type="button" class="btn-close" data-bs-dismiss="alert"></button>
</div>
<?php endif; ?>

<form method="POST" novalidate>
    <?= csrf_field() ?>
    <div class="row g-4">

        <div class="col-lg-8">
            <div class="card mb-4" style="border-radius:12px;">
                <div class="card-header py-3 px-4">
                    <h6 class="mb-0 fw-semibold"><i class="fas fa-info-circle me-2 text-muted"></i>Exam Information</h6>
                </div>
                <div class="card-body p-4">
                    <div class="mb-3">
                        <label class="form-label fw-medium">Exam Title <span class="text-danger">*</span></label>
                        <input type="text" name="exam_title" class="form-control"
                               value="<?= old('exam_title', $exam['exam_title']) ?>"
                               maxlength="300" required>
                    </div>
                    <div class="mb-3">
                        <label class="form-label fw-medium">Level / Category</label>
                        <input type="text" name="exam_level" class="form-control"
                               value="<?= old('exam_level', $exam['exam_level'] ?? '') ?>"
                               maxlength="100">
                    </div>
                    <div class="mb-3">
                        <label class="form-label fw-medium">Notes</label>
                        <textarea name="notes" class="form-control" rows="3"><?= old('notes', $exam['notes'] ?? '') ?></textarea>
                    </div>
                </div>
            </div>

            <div class="card mb-4" style="border-radius:12px;">
                <div class="card-header py-3 px-4">
                    <h6 class="mb-0 fw-semibold">
                        <i class="fas fa-university me-2 text-muted"></i>Department &amp; Program
                    </h6>
                </div>
                <div class="card-body p-4">
                    <div class="row g-3 mb-2">
                        <div class="col-md-6">
                            <label class="form-label fw-medium">Department <span class="text-danger">*</span></label>
                            <select name="dept_id" id="dept_select" class="form-select" required>
                                <option value="">— Select Department —</option>
                                <?php foreach ($departments as $d): ?>
                                <option value="<?= $d['id'] ?>"
                                        data-faculty="<?= h($d['faculty_label']) ?>"
                                        <?= (int)$v_dept === (int)$d['id'] ? 'selected' : '' ?>>
                                    <?= h($d['name']) ?>
                                </option>
                                <?php endforeach; ?>
                            </select>
                        </div>
                        <div class="col-md-6">
                            <label class="form-label fw-medium">Program</label>
                            <select name="program_id" id="prog_select" class="form-select">
                                <option value="">— None / All Programs —</option>
                                <?php foreach ($programs as $p): ?>
                                <option value="<?= $p['id'] ?>" <?= (int)$v_prog === (int)$p['id'] ? 'selected' : '' ?>>
                                    <?= h($p['program_name']) ?>
                                </option>
                                <?php endforeach; ?>
                            </select>
                        </div>
                    </div>
                    <div id="faculty_preview" class="text-muted small">
                        <?php
                        foreach ($departments as $d) {
                            if ((int)$d['id'] === (int)$v_dept) {
                                echo h($d['faculty_label']);
                                break;
                            }
                        }
                        ?>
                    </div>
                </div>
            </div>
        </div>

        <div class="col-lg-4">
            <div class="card mb-4" style="border-radius:12px;">
                <div class="card-header py-3 px-4">
                    <h6 class="mb-0 fw-semibold"><i class="fas fa-calendar-alt me-2 text-muted"></i>Batch &amp; Semesters</h6>
                </div>
                <div class="card-body p-4">
                    <div class="mb-3">
                        <label class="form-label fw-medium">Batch</label>
                        <input type="text" name="batch" class="form-control"
                               value="<?= old('batch', $exam['batch'] ?? '') ?>" maxlength="50"
                               placeholder="e.g. 52nd" list="batch_list">
                        <datalist id="batch_list">
                            <?php
                            $batches = db()->query(
                                "SELECT DISTINCT batch FROM students WHERE batch IS NOT NULL AND batch != ''
                                 ORDER BY batch DESC"
                            )->fetchAll(PDO::FETCH_COLUMN);
                            foreach ($batches as $b): ?>
                            <option value="<?= h($b) ?>">
                            <?php endforeach; ?>
                        </datalist>
                    </div>
                    <div class="mb-3">
                        <label class="form-label fw-medium">Enrollment Semester</label>
                        <input type="text" name="enrollment_semester" class="form-control"
                               value="<?= old('enrollment_semester', $exam['enrollment_semester'] ?? '') ?>"
                               maxlength="50" list="sem_list">
                        <datalist id="sem_list">
                            <?php foreach (array_reverse($semesters) as $s): ?>
                            <option value="<?= h($s) ?>">
                            <?php endforeach; ?>
                        </datalist>
                    </div>
                    <div class="mb-4">
                        <label class="form-label fw-medium">Completion Semester</label>
                        <input type="text" name="completion_semester" class="form-control"
                               value="<?= old('completion_semester', $exam['completion_semester'] ?? '') ?>"
                               maxlength="50" list="sem_list">
                    </div>
                    <div class="form-check mb-4">
                        <input class="form-check-input" type="checkbox" name="is_published"
                               id="is_published" value="1"
                               <?= (int)(old('is_published') ?: $exam['is_published']) ? 'checked' : '' ?>>
                        <label class="form-check-label" for="is_published">Published</label>
                    </div>
                    <div class="d-grid gap-2">
                        <button type="submit" class="btn btn-primary" style="border-radius:10px;">
                            <i class="fas fa-save me-1"></i> Save Changes
                        </button>
                        <a href="<?= APP_URL ?>/results/view.php?id=<?= $id ?>"
                           class="btn btn-light" style="border-radius:10px;">Cancel</a>
                    </div>
                </div>
            </div>
        </div>

    </div>
</form>

<script>
(function () {
    var deptSel    = document.getElementById('dept_select');
    var progSel    = document.getElementById('prog_select');
    var facPreview = document.getElementById('faculty_preview');
    var savedProg  = <?= (int)$v_prog ?>;

    deptSel.addEventListener('change', function () {
        var sel = this.options[this.selectedIndex];
        facPreview.textContent = sel && sel.dataset.faculty ? sel.dataset.faculty : '';
        var deptId = this.value;
        progSel.innerHTML = '<option value="">Loading…</option>';
        progSel.disabled  = true;
        if (!deptId) {
            progSel.innerHTML = '<option value="">— None / All Programs —</option>';
            progSel.disabled  = false;
            return;
        }
        fetch('<?= APP_URL ?>/results/get-programs.php?dept_id=' + encodeURIComponent(deptId))
            .then(function (r) { return r.json(); })
            .then(function (data) {
                progSel.innerHTML = '<option value="">— None / All Programs —</option>';
                data.forEach(function (p) {
                    var opt = document.createElement('option');
                    opt.value = p.id;
                    opt.textContent = p.program_name;
                    if (p.id == savedProg) opt.selected = true;
                    progSel.appendChild(opt);
                });
                progSel.disabled = false;
            });
    });
})();
</script>

<?php require_once __DIR__ . '/../includes/footer.php'; ?>
