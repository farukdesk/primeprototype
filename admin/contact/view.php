...
$id      = (int)($_GET['id'] ?? 0);
$contact = contact_get_message($id);
...
$page_title = 'Message from ' . h($contact['name']);
    ...
    <div class="col-lg-8">
        <div class="card" style="border-radius:14px;border:none;box-shadow:0 4px 24px rgba(0,0,0,.06);">
            <div class="card-body p-4">
                <h5 style="color:#1a2e5a;font-weight:700;margin-bottom:4px;"><?= h($contact['subject']) ?></h5>
                <p style="font-size:.82rem;color:#9ca3af;margin-bottom:24px;">
                    Received on <?= $contact['created_at'] ? date('l, d F Y \a\t h:i A', strtotime($contact['created_at'])) : 'Unknown date' ?>
                </p>
                <hr style="border-color:#f0f0f0;">
                <div style="font-size:.95rem;color:#374151;line-height:1.8;white-space:pre-wrap;margin-top:20px;"><?= h($contact['message']) ?></div>
            </div>
        </div>
    </div>
        ...
        <div class="card" style="border-radius:14px;border:none;box-shadow:0 4px 24px rgba(0,0,0,.06);">
            <div class="card-body p-4">
                ...
                <div class="d-flex align-items-center gap-3 mb-4">
                    ...
                    <div>
                        <div style="font-weight:700;color:#1a2e5a;"><?= h($contact['name']) ?></div>
                ...
                <div style="font-size:.875rem;color:#374151;">
                    <div class="d-flex gap-2 mb-3">
                        ...
                        <a href="mailto:<?= h($contact['email']) ?>" style="color:#2563eb;text-decoration:none;word-break:break-all;">
                            <?= h($contact['email']) ?>
                        </a>
                    </div>
                    ...
                    <div class="d-flex gap-2 mb-3">
                        ...
                        <a href="tel:<?= h($contact['phone']) ?>" style="color:#374151;text-decoration:none;">
                            <?= h($contact['phone']) ?>
                        </a>
                    </div>
                    <?php endif; ?>
                    <div class="d-flex gap-2">
                        ...
                        <span><?= $contact['created_at'] ? date('d M Y, h:i A', strtotime($contact['created_at'])) : 'Unknown date' ?></span>
                    </div>
                </div>

                <hr style="border-color:#f0f0f0;margin:20px 0;">

                <a href="mailto:<?= h($contact['email']) ?>?subject=Re: <?= rawurlencode($contact['subject']) ?>"
