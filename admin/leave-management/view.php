...
// Always show BOTH Casual and Sick balances, even when this request is for
// another leave type or the balance is exhausted.
$bal_year = (int)date('Y', strtotime($req['start_date']));
$bal      = lm_get_balance((int)$req['user_id'], $bal_year);
$bal_rows = [];
foreach (LM_BALANCE_CATEGORIES as $bc) {
    $rem = (float)$bal[$bc . '_remaining'];
    $tot = (float)$bal[$bc . '_total'];
    // An approved balance-consuming request is already deducted – show the
    // figure as it stood BEFORE this approval.
    $before = ($req['status'] === 'approved' && $req['category'] === $bc) ? $rem + (float)$req['days'] : $rem;
    $bal_rows[$bc] = [
        'total'  => $tot,
        'before' => $before,
        'after'  => $before - (float)$req['days'],
    ];
}
?>
            ...
            <div class="card-body p-4">
                <table class="table table-borderless mb-0">
                    <tbody>
                        ...
                        <?php foreach ($bal_rows as $bc => $b): ?>
                        <tr>
                            <th class="text-muted"><?= h(lm_category_label($bc)) ?> Left (<?= $bal_year ?>)</th>
                            <td>
                                <strong class="<?= $b['before'] <= 0 ? 'text-danger' : '' ?>"><?= $fmt($b['before']) ?></strong>
                                of <?= $fmt($b['total']) ?> day(s) left before this approval
                                <?php if ($req['category'] === $bc && $req['status'] === 'pending'): ?>
                                    <i class="fas fa-arrow-right mx-1 text-muted"></i>
                                    <strong class="<?= $b['after'] < 0 ? 'text-danger' : 'text-success' ?>"><?= $fmt($b['after']) ?></strong> day(s) will be left after approval
                                    <?php if ($b['after'] < 0): ?>
                                    ...
                                    <?php endif; ?>
                                <?php elseif ($req['category'] === $bc && $req['status'] === 'approved'): ?>
                                    <i class="fas fa-arrow-right mx-1 text-muted"></i>
                                    <strong class="<?= $b['after'] < 0 ? 'text-danger' : 'text-success' ?>"><?= $fmt($b['after']) ?></strong> day(s) left after this approval
                                <?php endif; ?>
                            </td>
                        </tr>
                        <?php endforeach; ?>
