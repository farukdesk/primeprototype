...
require_access('dept-prime-pride', 'can_create');
...
$page_title = 'Add Prime Pride – ' . $dept['name'];
$errors = [];
clear_old();
    ...
    if (empty($errors)) {
        db()->prepare(
            'INSERT INTO dept_prime_pride
             (dept_id, title, description, image, student_name, position, company, linkedin_url, batch_year, sort_order, is_active)
             VALUES (?,?,?,?,?,?,?,?,?,?,?)'
        )->execute([$dept_id, $title, $description ?: null, $image,
                    $student_name ?: null, $position ?: null,
                    $company ?: null, $linkedin_url ?: null,
                    $batch_year ?: null, $sort_order, $is_active]);

        flash_set('success', "Prime Pride entry <strong>" . h($title) . "</strong> added.");
        redirect(APP_URL . '/departments/prime-pride/index.php?dept_id=' . $dept_id);
    }

    save_old(compact('title','description','student_name','position','company','linkedin_url','batch_year','sort_order'));
}
...
<div class="card">
    <div class="card-header py-3 px-4">
        <h6 class="mb-0 fw-semibold"><i class="fas fa-star me-2 text-muted"></i>Add Prime Pride</h6>
    </div>
            ...
            <div class="row g-3">
                ...
                <div class="col-md-6">
                    ...
                    <input type="text" name="student_name" class="form-control" style="border-radius:10px;"
                           value="<?= old('student_name') ?>" maxlength="200">
                </div>
                <div class="col-md-6">
                ...
                <div class="col-md-6">
                    ...
                    <input type="text" name="company" class="form-control" style="border-radius:10px;"
                           value="<?= old('company') ?>" maxlength="200" placeholder="e.g. Google, Microsoft">
                </div>
                <div class="col-md-6">
                ...
                <div class="col-md-6">
                    ...
                    <input type="text" name="batch_year" class="form-control" style="border-radius:10px;"
                           value="<?= old('batch_year') ?>" maxlength="50" placeholder="e.g. 2020">
                </div>
                <div class="col-md-6">
