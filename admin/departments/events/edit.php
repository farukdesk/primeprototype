...
require_access('dept-events', 'can_edit');
...
$stmt->execute([$id]);
$event = $stmt->fetch();
if (!$event) { flash_set('error', 'Event not found.'); redirect(APP_URL . '/departments/index.php'); }

$dept_id = $dept_id ?: (int)$event['dept_id'];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        db()->prepare(
            'UPDATE dept_events SET
             title=?, event_date=?, location=?, description=?, link_url=?, is_active=?
             WHERE id=?'
        )->execute([$title, $event_date ?: null, $location ?: null,
                    $description ?: null, $link_url ?: null, $is_active, $id]);

        flash_set('success', "Event <strong>" . h($title) . "</strong> updated.");
        redirect(APP_URL . '/departments/events/index.php?dept_id=' . $dept_id);
    }

    $event = array_merge($event, compact('title','event_date','location','description','link_url','is_active'));
}
...
<div class="card">
    <div class="card-header py-3 px-4">
        <h6 class="mb-0 fw-semibold"><i class="fas fa-calendar-edit me-2 text-muted"></i>Edit Event</h6>
    </div>
            ...
            <div class="row g-3">
                <div class="col-12">
                    ...
                    <input type="text" name="title" class="form-control" style="border-radius:10px;"
                           value="<?= h($event['title']) ?>" required maxlength="300">
                </div>
                <div class="col-md-6">
                ...
                <div class="col-md-6">
                    ...
                    <input type="text" name="location" class="form-control" style="border-radius:10px;"
                           value="<?= h($event['location'] ?? '') ?>" maxlength="300">
                </div>
                <div class="col-12">
                    <label class="form-label fw-medium">Description</label>
                    <textarea name="description" class="form-control" style="border-radius:10px;" rows="4"><?= h($event['description'] ?? '') ?></textarea>
                </div>
                <div class="col-md-8">
                    <label class="form-label fw-medium">Link URL</label>
                    <input type="url" name="link_url" class="form-control" style="border-radius:10px;"
                           value="<?= h($event['link_url'] ?? '') ?>" maxlength="500">
                </div>
