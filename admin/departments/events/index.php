...
$events = db()->prepare(
    'SELECT * FROM dept_events WHERE dept_id = ? ORDER BY event_date DESC, id DESC'
);
$events->execute([$dept_id]);
$events = $events->fetchAll();

$page_title = 'Events – ' . $dept['name'];
...
<div class="d-flex justify-content-between align-items-center mb-4">
    ...
    <div class="d-flex gap-2">
        ...
        <a href="<?= APP_URL ?>/departments/events/create.php?dept_id=<?= $dept_id ?>" class="btn btn-primary btn-sm" style="border-radius:10px;">
...
<div class="card">
    <div class="card-header py-3 px-4 d-flex align-items-center justify-content-between">
        <h6 class="mb-0 fw-semibold"><i class="fas fa-calendar-alt me-2 text-muted"></i>Events</h6>
        <span class="badge bg-primary bg-opacity-10 text-primary"><?= count($events) ?> total</span>
    </div>
            ...
            <table class="table table-hover mb-0">
                ...
                <?php else: ?>
                    ...
                    <tr>
                        ...
                        <td><?= h($ev['title']) ?></td>
                        <td><?= $ev['event_date'] ? date('M d, Y', strtotime($ev['event_date'])) : '—' ?></td>
                        <td><?= h($ev['location'] ?? '—') ?></td>
                        <td>
                        ...
                        <td class="text-end pe-4">
                            <div class="d-flex gap-1 justify-content-end">
                                <a href="<?= APP_URL ?>/departments/events/edit.php?id=<?= $ev['id'] ?>&dept_id=<?= $dept_id ?>"
                                   class="btn btn-sm btn-outline-primary" style="border-radius:7px;" title="Edit">
                                    <i class="fas fa-edit"></i>
                                </a>
                                <form method="POST" action="<?= APP_URL ?>/departments/events/delete.php"
                                      onsubmit="return confirm('Delete event &quot;<?= h(addslashes($ev['title'])) ?>&quot;?');">
                                    <?= csrf_field() ?>
