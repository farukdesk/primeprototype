...
require_access('dept-facilities', 'can_edit');
...
$stmt->execute([$id]);
$facility = $stmt->fetch();
if (!$facility) { flash_set('error', 'Facility not found.'); redirect(APP_URL . '/departments/index.php'); }

$dept_id = $dept_id ?: (int)$facility['dept_id'];
...
$page_title = 'Edit Facility – ' . $facility['title'];
$errors = [];
clear_old();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $icon        = trim($_POST['icon']        ?? 'fas fa-desktop');
    ...
    if (!empty($_FILES['image']['name'])) {
        ...
        if ($uploaded === false) {
            ...
            if ($facility['image']) {
                $old = UPLOAD_DIR . '/departments/' . $facility['image'];
    ...
    if (empty($errors)) {
        db()->prepare(
            'UPDATE dept_facilities SET
             title=?, description=?, icon=?, image=?, sort_order=?, is_active=?
             WHERE id=?'
        )->execute([$title, $description ?: null,
                    $icon ?: 'fas fa-desktop', $image, $sort_order, $is_active, $id]);

        flash_set('success', "Facility <strong>" . h($title) . "</strong> updated.");
        redirect(APP_URL . '/departments/facilities/index.php?dept_id=' . $dept_id);
    }

    $facility = array_merge($facility, compact('title','description','icon','sort_order','is_active'));
...
<div class="d-flex justify-content-between align-items-center mb-4">
    <nav aria-label="breadcrumb">
        <ol class="breadcrumb mb-0">
            ...
            <li class="breadcrumb-item"><a href="<?= APP_URL ?>/departments/facilities/index.php?dept_id=<?= $dept_id ?>">Facilities</a></li>
...
<div class="card">
    <div class="card-header py-3 px-4">
        <h6 class="mb-0 fw-semibold"><i class="fas fa-edit me-2 text-muted"></i>Edit Facility</h6>
    </div>
    <div class="card-body p-4">
        <form method="POST" enctype="multipart/form-data" novalidate>
            ...
            <div class="row g-3">
                <div class="col-md-8">
                    ...
                    <input type="text" name="title" class="form-control" style="border-radius:10px;"
                           value="<?= h($facility['title']) ?>" required maxlength="300">
                </div>
                <div class="col-md-4">
                    <label class="form-label fw-medium">Icon</label>
                    <input type="text" name="icon" class="form-control" style="border-radius:10px;"
                           value="<?= h($facility['icon'] ?? 'fas fa-desktop') ?>" maxlength="100">
                </div>
                <div class="col-12">
                    <label class="form-label fw-medium">Description</label>
                    <textarea name="description" class="form-control" style="border-radius:10px;" rows="4"><?= h($facility['description'] ?? '') ?></textarea>
                </div>
                <div class="col-md-6">
                    <label class="form-label fw-medium">Image</label>
                    <?php if ($facility['image']): ?>
                    <div class="mb-2">
                        <img src="<?= UPLOAD_URL ?>/departments/<?= h($facility['image']) ?>"
                ...
                <div class="col-md-3">
                    ...
                    <input type="number" name="sort_order" class="form-control" style="border-radius:10px;"
                           value="<?= (int)$facility['sort_order'] ?>" min="0">
                </div>
            ...
            <div class="d-flex gap-2 mt-4">
                ...
                <a href="<?= APP_URL ?>/departments/facilities/index.php?dept_id=<?= $dept_id ?>"
