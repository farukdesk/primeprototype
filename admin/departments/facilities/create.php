...
require_access('dept-facilities', 'can_create');
...
$page_title = 'Add Facility – ' . $dept['name'];
$errors = [];
clear_old();
    ...
    if (empty($errors)) {
        db()->prepare(
            'INSERT INTO dept_facilities
             (dept_id, title, description, icon, image, sort_order, is_active)
             VALUES (?,?,?,?,?,?,?)'
        )->execute([$dept_id, $title, $description ?: null,
                    $icon ?: 'fas fa-desktop', $image, $sort_order, $is_active]);

        flash_set('success', "Facility <strong>" . h($title) . "</strong> added.");
        redirect(APP_URL . '/departments/facilities/index.php?dept_id=' . $dept_id);
    }

    save_old(compact('title','description','icon','sort_order'));
}
...
<div class="card">
    <div class="card-header py-3 px-4">
        <h6 class="mb-0 fw-semibold"><i class="fas fa-building me-2 text-muted"></i>Add Facility</h6>
    </div>
            ...
            <div class="row g-3">
                ...
                <div class="col-md-4">
                    ...
                    <input type="text" name="icon" class="form-control" style="border-radius:10px;"
                           value="<?= old('icon', 'fas fa-desktop') ?>" maxlength="100">
                    <small class="text-muted">e.g. <code>fas fa-desktop</code></small>
                </div>
                <div class="col-12">
