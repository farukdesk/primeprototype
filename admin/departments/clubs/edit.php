...
require_access('dept-clubs', 'can_edit');
...
$stmt->execute([$id]);
$club = $stmt->fetch();
if (!$club) { flash_set('error', 'Club not found.'); redirect(APP_URL . '/departments/index.php'); }

$dept_id = $dept_id ?: (int)$club['dept_id'];
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!empty($_FILES['logo']['name'])) {
        ...
        if ($uploaded === false) {
            ...
            if ($club['logo']) {
                $old = UPLOAD_DIR . '/departments/' . $club['logo'];
    ...
    if (empty($errors)) {
        db()->prepare(
            'UPDATE dept_clubs SET
             name=?, description=?, logo=?, president_name=?, email=?, sort_order=?, is_active=?
             WHERE id=?'
        )->execute([$name, $description ?: null, $logo,
                    $president_name ?: null, $email ?: null, $sort_order, $is_active, $id]);

        flash_set('success', "Club <strong>" . h($name) . "</strong> updated.");
        redirect(APP_URL . '/departments/clubs/index.php?dept_id=' . $dept_id);
    }

    $club = array_merge($club, compact('name','description','president_name','email','sort_order','is_active'));
    $club['logo'] = $logo;
}
...
<div class="card">
    <div class="card-header py-3 px-4">
        <h6 class="mb-0 fw-semibold"><i class="fas fa-edit me-2 text-muted"></i>Edit: <?= h($club['name']) ?></h6>
    </div>
            ...
            <div class="row g-3">
                <div class="col-md-6">
                    ...
                    <input type="text" name="name" class="form-control" style="border-radius:10px;"
                           value="<?= h($club['name']) ?>" required maxlength="200">
                </div>
                <div class="col-md-6">
                    ...
                    <input type="text" name="president_name" class="form-control" style="border-radius:10px;"
                           value="<?= h($club['president_name'] ?? '') ?>" maxlength="200">
                </div>
                <div class="col-md-6">
                    <label class="form-label fw-medium">Email</label>
                    <input type="email" name="email" class="form-control" style="border-radius:10px;"
                           value="<?= h($club['email'] ?? '') ?>" maxlength="200">
                </div>
                <div class="col-md-6">
                    <label class="form-label fw-medium">Logo</label>
                    <?php if ($club['logo']): ?>
                    <div class="mb-2">
                        <img src="<?= UPLOAD_URL ?>/departments/<?= h($club['logo']) ?>"
                             alt="" style="height:60px;border-radius:8px;object-fit:cover;">
                    </div>
                    <?php endif; ?>
                ...
                <div class="col-12">
                    ...
                    <textarea name="description" class="form-control" style="border-radius:10px;" rows="4"><?= h($club['description'] ?? '') ?></textarea>
                </div>
                <div class="col-md-6">
                    <label class="form-label fw-medium">Sort Order</label>
                    <input type="number" name="sort_order" class="form-control" style="border-radius:10px;"
                           value="<?= (int)$club['sort_order'] ?>" min="0">
                </div>
