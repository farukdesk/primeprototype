...
require_access('dept-academic-programs', 'can_edit');
...
$stmt->execute([$id]);
$item = $stmt->fetch();
if (!$item) { flash_set('error', 'Criterion not found.'); redirect(APP_URL . '/departments/index.php'); }
...
$page_title = 'Edit Eligibility Criterion';
$errors = [];
clear_old();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        db()->prepare(
            'UPDATE program_eligibility_criteria SET category=?, criterion=?, sort_order=?, is_active=? WHERE id=?'
        )->execute([$category, $criterion, $sort_order, $is_active, $id]);

        flash_set('success', "Eligibility criterion updated.");
        redirect(APP_URL . '/departments/academic-programs/eligibility/index.php?program_id=' . $program_id);
    }

    $item = array_merge($item, compact('category','criterion','sort_order','is_active'));
}
...
<div class="card">
    <div class="card-header py-3 px-4">
        <h6 class="mb-0 fw-semibold"><i class="fas fa-edit me-2 text-muted"></i>Edit Eligibility Criterion</h6>
    </div>
        ...
        <form method="POST" novalidate>
            ...
            <div class="row g-3">
                ...
                <div class="col-12">
                    ...
                    <textarea name="criterion" class="form-control" style="border-radius:10px;" rows="4" required><?= h($item['criterion']) ?></textarea>
                </div>
            </div>
