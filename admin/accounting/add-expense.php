...
$expense_accounts = acc_expense_accounts();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!$expense_account_id)     $errors[] = 'Please select the expense category.';
    if (!$cash_account_id)        $errors[] = 'Please select the payment account.';
    if (!$date)                   $errors[] = 'Date is required.';
    if ($cash_account_id === $expense_account_id) $errors[] = 'Payment account and expense category cannot be the same.';

    if (empty($errors)) {
        try {
            $vid = acc_add_expense($amount, $expense_account_id, $cash_account_id, $date, $reference, $narration);
            flash_set('success', 'Expense recorded successfully. <a href="' . APP_URL . '/accounting/voucher-view.php?id=' . $vid . '" class="alert-link">View Voucher</a>');
            redirect(APP_URL . '/accounting/add-expense.php');
...
<div class="d-flex align-items-center justify-content-between mb-4 flex-wrap gap-2">
    <div>
        <h4 class="mb-0 fw-semibold"><i class="fas fa-receipt me-2 text-danger"></i>Add Expense</h4>
        ...
        <div class="card border-0 shadow-sm">
            <div class="card-header py-3 px-4">
                <div class="d-flex align-items-center gap-2">
                    <span class="badge bg-danger p-2"><i class="fas fa-receipt"></i></span>
                    <div>
                        <div class="fw-semibold">Payment Voucher</div>
                        <div class="text-muted small">Records money paid out (expense)</div>
                    </div>
                </div>
            </div>
                ...
                <form method="post">
                    ...
                    <div class="row g-3">
                        ...
                        <div class="col-md-6">
                            ...
                            <div class="form-text">What was this expense for?</div>
                        </div>

                        <div class="col-md-6">
                            <label class="form-label fw-semibold">Pay From <span class="text-danger">*</span></label>
                            ...
                            <div class="form-text">Cash or bank account to pay from</div>
                        </div>

                        <div class="col-12">
                            <label class="form-label fw-semibold">Reference / Bill # <small class="text-muted fw-normal">(optional)</small></label>
                            <input type="text" name="reference" class="form-control"
                                   placeholder="e.g. BILL-001, Cheque #, Invoice #"
                                   value="<?= old('reference') ?>">
                        </div>
                        ...
                        <div class="col-12">
                            ...
                            <textarea name="narration" class="form-control" rows="2"
                                      placeholder="e.g. Monthly electricity bill – October 2025"><?= old('narration') ?></textarea>
                        </div>
                    </div>
                    ...
                    <div class="alert alert-light border mt-4 small">
                        ...
                        <span class="text-success">Debit</span> the expense category &amp;
                        <span class="text-danger">Credit</span> the payment account automatically.
                    </div>

                    <div class="d-flex gap-2 mt-2">
                        <button type="submit" class="btn btn-danger"><i class="fas fa-check me-1"></i> Post Payment Voucher</button>
