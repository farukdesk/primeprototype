...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        db()->prepare(
            'UPDATE ei_faculty SET dept_id=?, name=?, designation=?, gender=?, weekend_available=?, weekend_days=?, contact_number=?, signature=?, is_active=? WHERE id=?'
        )->execute([$dept_id, $name, $designation ?: null, $gender, $weekend_available, $weekend_days, $contact_number ?: null, $signature, $is_active, $fid]);
