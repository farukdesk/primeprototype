...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    // Parse subject rows: rows[][curriculum_id] + rows[][teacher_ids][]
    ...
    if (empty($errors)) {
        ...
        co_save_subjects_teachers($offer_id, $rows);

        $sub_count = count($rows);
        log_change(
            'course-offer', 'CREATE', $offer_id,
            'Offer #' . $offer_id,
            null, null, null,
            'Course offer created with ' . $sub_count . ' subject(s) for batch #' . $batch_id
        );

        flash_set('success', 'Course offer created with <strong>' . $sub_count . '</strong> subject(s).');
        redirect(APP_URL . '/course-offer/index.php');
    }
...
foreach ($departments as $d) {
    if ((int)$d['id'] === $old_dept) { $old_dept_name = $d['name']; break; }
}
$old_dept_bi = co_dept_is_bi_semester($old_dept_name);
$intake_opts = co_academic_intake_options($old_dept_bi);

$programs = $old_dept > 0 ? co_programs($old_dept) : [];

// Pre-load subject details for repopulation after validation error
$pre_rows = [];
foreach ($old_rows as $row) {
    ...
    $teacher_ids = array_values(array_filter(array_map('intval', (array)($row['teacher_ids'] ?? []))));
    $pre_teachers = [];
...
<!-- Pre-populated rows data for TomSelect init -->
...
// ── TomSelect instances registry ──────────────────────────────────────────────
var tsSubjectMap = {};   // ri → TomSelect instance (subject)
var tsTeacherMap = {};   // ri → TomSelect instance (teachers)
...
function applyIntakeOptions(deptId) {
    ...
    // Preserve the current selection if it is still valid for this calendar.
