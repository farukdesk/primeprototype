...
$page_title = 'Edit Legal Service';
$errors = [];

$id = (int)($_GET['id'] ?? 0);
$row = null;
try {
    $stmt = db()->prepare('SELECT * FROM ll_services WHERE id = ?');
    $stmt->execute([$id]);
    $row = $stmt->fetch();
} catch (Throwable $e) {}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        db()->prepare(
            'UPDATE ll_services SET title=?, description=?, icon=?, sort_order=?, is_active=?
             WHERE id=?'
        )->execute([
...
<form method="POST" novalidate>
    ...
    <div class="card mb-4" style="border-radius:12px;">
        <div class="card-header py-3 px-4">
            <h6 class="mb-0 fw-semibold"><i class="fas fa-balance-scale me-2 text-muted"></i>Edit Service</h6>
        </div>
        <div class="card-body p-4">
            <div class="row g-3">
                ...
                <div class="col-12">
                    ...
                    <textarea name="description" class="form-control" rows="4"><?= h($f['description']) ?></textarea>
                </div>
                <div class="col-12">
