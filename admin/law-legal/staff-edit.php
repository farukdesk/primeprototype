...
$page_title = 'Edit Legal Staff Member';
$errors = [];

$id = (int)($_GET['id'] ?? 0);
$row = null;
try {
    $row = db()->prepare('SELECT * FROM ll_staff WHERE id = ?');
    $row->execute([$id]);
    $row = $row->fetch();
} catch (Throwable $e) {}
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        db()->prepare(
            'UPDATE ll_staff SET name=?, designation=?, email=?, phone=?, bio=?, photo=?, sort_order=?, is_active=?, updated_at=NOW()
             WHERE id=?'
        )->execute([
        ...
        flash_set('success', 'Staff member <strong>' . h($f['name']) . '</strong> updated.');
        redirect(APP_URL . '/law-legal/staff-index.php');
    }
}
...
<form method="POST" enctype="multipart/form-data" novalidate>
    ...
    <div class="card mb-4" style="border-radius:12px;">
        <div class="card-header py-3 px-4">
            <h6 class="mb-0 fw-semibold"><i class="fas fa-user-edit me-2 text-muted"></i>Edit Staff Member</h6>
        </div>
        <div class="card-body p-4">
            ...
            <div class="mb-4">
                ...
                <div class="d-flex align-items-center gap-3">
                    <?php if ($row['photo']): ?>
                    <img id="ll-staff-preview" src="<?= UPLOAD_URL ?>/law-legal/<?= h($row['photo']) ?>"
            ...
            <div class="row g-3">
                ...
                <div class="col-12">
                    ...
                    <textarea name="bio" class="form-control" rows="4"><?= h($f['bio']) ?></textarea>
                </div>
                <div class="col-12">
