...
$page_title = 'COE Staff Directory';

$staff = [];
try {
    $staff = db()->query('SELECT * FROM coe_staff ORDER BY sort_order, id')->fetchAll();
} catch (Throwable $e) {}
...
<div class="d-flex justify-content-between align-items-center mb-4">
    ...
    <a href="<?= APP_URL ?>/office-of-coe/staff-create.php" class="btn btn-sm btn-primary" style="border-radius:8px;">
...
<div class="card" style="border-radius:12px;">
    ...
    <div class="table-responsive">
        <table class="table table-hover mb-0">
            ...
            <tbody>
                <?php if (empty($staff)): ?>
                <tr><td colspan="7" class="text-center py-4 text-muted">No staff members found. <a href="<?= APP_URL ?>/office-of-coe/staff-create.php">Add one.</a></td></tr>
                <?php else: ?>
                ...
                <tr>
                    ...
                    <td style="padding:12px 16px;">
                        <div class="d-flex align-items-center gap-2">
                            ...
                            <img src="<?= UPLOAD_URL ?>/office-of-coe/<?= h($st['photo']) ?>"
                    ...
                    <td style="padding:12px 16px;font-size:.85rem;">
                        <?php if ($st['phone']): ?><?= h($st['phone']) ?><?php if (!empty($st['phone_2'])): ?><br><span style="color:#94a3b8;"><?= h($st['phone_2']) ?></span><?php endif; ?><?php else: ?>—<?php endif; ?>
                    </td>
                    <td style="padding:12px 16px;">
                    ...
                    <td style="padding:12px 16px;">
                        <div class="d-flex gap-1">
                            ...
                            <a href="<?= APP_URL ?>/office-of-coe/staff-edit.php?id=<?= $st['id'] ?>"
                               class="btn btn-xs btn-outline-primary" style="padding:3px 8px;font-size:.75rem;border-radius:6px;" title="Edit">
                                <i class="fas fa-edit"></i>
                            </a>
                            <?php endif; ?>
                            <?php if (can_access('office-of-coe', 'can_delete')): ?>
                            <form method="POST" action="<?= APP_URL ?>/office-of-coe/staff-delete.php"
