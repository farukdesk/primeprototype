...
$id = (int)($_GET['id'] ?? 0);
if ($id <= 0) { redirect(APP_URL . '/office-of-coe/staff-index.php'); }

$st = null;
try {
    $st = db()->prepare('SELECT * FROM coe_staff WHERE id = ?');
    $st->execute([$id]);
    $st = $st->fetch();
} catch (Throwable $e) {}
if (!$st) { flash_set('error', 'Staff member not found.'); redirect(APP_URL . '/office-of-coe/staff-index.php'); }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['remove_photo']) && $_POST['remove_photo'] === '1') {
        if ($photo) { $old = UPLOAD_DIR . '/office-of-coe/' . $photo; if (is_file($old)) @unlink($old); }
        $photo = '';
    }

    if (empty($errors)) {
        db()->prepare(
            'UPDATE coe_staff SET name=?, title=?, email_1=?, email_2=?, phone=?, phone_2=?, photo=?, sort_order=?, is_active=? WHERE id=?'
        )->execute([$name, $title, $email_1, $email_2, $phone, $phone_2, $photo, $sort_order, $is_active, $id]);
    ...
    $st = array_merge($st, compact('name','title','email_1','email_2','phone','phone_2','sort_order','is_active','photo'));
}
        ...
        <div class="card-body p-4">
            ...
            <div class="mb-4">
                ...
                <div class="d-flex align-items-center gap-3 mb-2">
                    ...
                    <img src="<?= UPLOAD_URL ?>/office-of-coe/<?= h($st['photo']) ?>" id="photo-preview"
            ...
            <div class="row g-3">
                ...
                <div class="col-md-6"><label class="form-label fw-medium">Phone 2</label>
                    <input type="text" name="phone_2" class="form-control" value="<?= h($st['phone_2'] ?? '') ?>"></div>
                <div class="col-12">
