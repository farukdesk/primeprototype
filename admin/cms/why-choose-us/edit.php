...
if ($id) {
    $stmt = db()->prepare('SELECT * FROM cms_features WHERE id = ?');
    $stmt->execute([$id]);
    $item = $stmt->fetch();
}
if (!$item) {
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        db()->prepare(
            'UPDATE cms_features
             SET icon=?, title=?, description=?, sort_order=?, is_active=?, updated_at=NOW()
             WHERE id=?'
        )->execute([$icon ?: 'fas fa-star', $title, $description ?: null, $sort_order, $is_active, $id]);

        flash_set('success', 'Feature card <strong>' . h($title) . '</strong> updated.');
        redirect(APP_URL . '/cms/why-choose-us/index.php');
    }

    $item = array_merge($item, compact('icon', 'title', 'description', 'sort_order', 'is_active'));
}
...
<div class="card">
    <div class="card-header py-3 px-4">
        <h6 class="mb-0 fw-semibold"><i class="fas fa-edit me-2 text-muted"></i>Edit Feature Card</h6>
    </div>
    <div class="card-body p-4">
        ...
        <form method="POST" novalidate>
            ...
            <div class="mb-3">
                ...
                <div class="input-group">
                    <span class="input-group-text" id="iconPreview" style="min-width:42px;font-size:1.1rem;">
                        <i id="iconPreviewEl" class="<?= h($item['icon']) ?>"></i>
                    </span>
                    <input type="text" name="icon" id="iconInput" class="form-control"
                           value="<?= h($item['icon']) ?>" placeholder="fas fa-graduation-cap" maxlength="100">
                </div>
            ...
            <div class="mb-3">
                ...
                <textarea name="description" class="form-control" rows="3"><?= h($item['description'] ?? '') ?></textarea>
            </div>
