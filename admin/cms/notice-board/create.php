...
const NBC_EXTS = ['pdf','doc','docx','xls','xlsx','jpg','jpeg','png','zip','txt'];
const NBC_MIMES = [
    'application/pdf',
    'application/msword',
...
function nbc_slug(string $title): string {
    $slug = mb_strtolower(trim($title));
    $slug = preg_replace('/[^a-z0-9]+/', '-', $slug);
    return trim($slug, '-') ?: 'notice';
}

function nbc_unique_notice_slug(string $base): string {
    $slug = $base;
    $i    = 2;
    $db   = db();
    while (true) {
        $st = $db->prepare('SELECT id FROM cms_notices WHERE slug = ?');
        $st->execute([$slug]);
        if (!$st->fetch()) break;
        $slug = $base . '-' . $i++;
    }
...
function nbc_unique_news_slug(string $base): string {
    $slug = $base;
    $i    = 2;
    $db   = db();
    while (true) {
...
function nbc_upload_attachment(array $file): string|false {
    if ($file['error'] !== UPLOAD_ERR_OK) return false;
    $ext = strtolower(pathinfo($file['name'], PATHINFO_EXTENSION));
    if (!in_array($ext, NBC_EXTS, true)) return false;
    $finfo = new finfo(FILEINFO_MIME_TYPE);
    $mime  = $finfo->file($file['tmp_name']);
    if (!in_array($mime, NBC_MIMES, true)) return false;
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (!empty($_FILES['attachment']['name'])) {
        ...
        $result = nbc_upload_attachment($file);
        if ($result === false) {
    ...
    if (empty($errors)) {
        $db          = db();
        $slug        = nbc_unique_notice_slug(nbc_slug($title));
        ...
        // Super admins may mirror the notice into Latest News immediately.
        if ($is_super && $publish_as_news) {
            $news_slug = nbc_unique_news_slug($slug);
            $db->prepare(
        ...
        if ($is_super) {
            // Notify all portal students when a notice is published on creation.
            if ($is_published) {
            ...
            // Notify super admins by email that a new notice awaits approval.
            require_once __DIR__ . '/../../includes/mailer.php';
            $supers = db()->query(
    ...
    save_old(compact('title','content','content_type','publish_as_news','is_published'));
}
...
<div class="alert alert-info py-2 mb-4" style="border-radius:10px;font-size:.875rem;">
    ...
    Your notice will be queued for super-admin approval and will not appear on the website immediately.
</div>
<?php endif; ?>
                ...
                <div class="card-body p-4">
                    ...
                    <div class="d-grid gap-2">
                        <button type="submit" class="btn btn-primary" style="border-radius:10px;">
                            ...
                            <?= $is_super ? 'Save Notice' : 'Submit for Approval' ?>
                        </button>
