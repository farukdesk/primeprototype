...
if ($is_super) {
    $pending_count = (int)db()->query(
        "SELECT COUNT(*) FROM cms_pending_changes WHERE module='notice' AND status='pending'"
    )->fetchColumn();
}
...
if ($is_super) {
    $prows = db()->query(
        "SELECT record_id, COUNT(*) AS cnt
         ...
         WHERE module='notice' AND status='pending'
         GROUP BY record_id"
    )->fetchAll();
    foreach ($prows as $pr) {
...
<div class="d-flex justify-content-between align-items-center mb-4">
    ...
    <div class="d-flex gap-2 align-items-center">
        ...
        <a href="<?= APP_URL ?>/cms/pending-changes/index.php?module=notice"
                ...
                <?php else: ?>
                    ...
                    <tr>
                        ...
                        <td>
                            ...
                            <?php endif; ?>
                            <?php if ($n_pending > 0): ?>
                            <a href="<?= APP_URL ?>/cms/pending-changes/index.php?module=notice&record_id=<?= $n['id'] ?>"
                               class="badge bg-warning text-dark text-decoration-none mt-1" style="font-size:.7rem;">
                        ...
                        <td>
                            <div class="d-flex gap-1 flex-wrap">
                                ...
                                <form method="POST" action="<?= APP_URL ?>/cms/notice-board/delete.php"
                                      onsubmit="return confirm('<?= $is_super ? 'Delete' : 'Request deletion of' ?> notice &quot;<?= h(addslashes($n['title'])) ?>&quot;?');">
                                    <?= csrf_field() ?>
                                    <input type="hidden" name="id" value="<?= $n['id'] ?>">
