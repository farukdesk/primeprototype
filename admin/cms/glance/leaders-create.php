...
$page_title = 'Add Leadership Card';
$errors     = [];
clear_old();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        db()->prepare(
            'INSERT INTO glance_leaders (name, role, bio, photo, sort_order, is_active)
             VALUES (?, ?, ?, ?, ?, ?)'
        )->execute([$name, $role, $bio, $photo, $sort_order, $is_active]);
        log_change('cms-glance', 'CREATE', (int)db()->lastInsertId(), $name, null, null, null, 'Leader created.');
        flash_set('success', 'Leader <strong>' . h($name) . '</strong> added.');
        redirect(APP_URL . '/cms/glance/index.php');
    }
    save_old(compact('name', 'role', 'bio', 'sort_order', 'is_active'));
}
...
<div class="d-flex justify-content-between align-items-center mb-4">
    <nav aria-label="breadcrumb">
        <ol class="breadcrumb mb-0">
            ...
            <li class="breadcrumb-item active">Add Leader</li>
        </ol>
    </nav>
</div>
...
<div class="card">
    <div class="card-header py-3 px-4"><h6 class="mb-0 fw-semibold"><i class="fas fa-user-tie me-2 text-muted"></i>New Leadership Card</h6></div>
        ...
        <form method="POST" enctype="multipart/form-data" novalidate>
            ...
            <div class="row g-3 mb-3">
                <div class="col-md-7">
                    ...
                    <input type="text" name="name" class="form-control" value="<?= h(old('name')) ?>" maxlength="200" required>
                </div>
                <div class="col-md-5">
                    <label class="form-label fw-medium">Role / Title <span class="text-danger">*</span></label>
                    <input type="text" name="role" class="form-control" value="<?= h(old('role')) ?>" maxlength="120" required placeholder="Treasurer">
                </div>
            </div>
            <div class="mb-3">
