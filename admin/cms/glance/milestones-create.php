...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        db()->prepare(
            'INSERT INTO glance_milestones (year_label, title, description, sort_order, is_active)
             VALUES (?, ?, ?, ?, ?)'
        )->execute([$year_label, $title, $description, $sort_order, $is_active]);
        log_change('cms-glance', 'CREATE', (int)db()->lastInsertId(), $title, null, null, null, 'Milestone created.');
        flash_set('success', 'Milestone <strong>' . h($title) . '</strong> added.');
        redirect(APP_URL . '/cms/glance/index.php');
    }
    save_old(compact('year_label', 'title', 'description', 'sort_order', 'is_active'));
}
...
<div class="card">
    <div class="card-header py-3 px-4"><h6 class="mb-0 fw-semibold"><i class="fas fa-history me-2 text-muted"></i>New Timeline Milestone</h6></div>
        ...
        <form method="POST" novalidate>
            ...
            <div class="row g-3 mb-3">
                <div class="col-md-4">
                    ...
                    <input type="text" name="year_label" class="form-control" value="<?= h(old('year_label')) ?>" maxlength="60" required placeholder="2002">
                    <div class="form-text">e.g. "2002" or "Growth Phase"</div>
                </div>
                <div class="col-md-8">
            ...
            <div class="mb-3">
                ...
                <textarea name="description" class="form-control" rows="4" required><?= h(old('description')) ?></textarea>
            </div>
            <div class="row g-3 mb-3">
                <div class="col-md-4">
