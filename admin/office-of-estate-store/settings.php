...
require_access('office-of-estate-store', 'can_edit');

$page_title = 'Office of the Estate & Store – Settings';
$errors     = [];
...
function es_upload_photo(array $file): string|false
{
    ...
    $dir = UPLOAD_DIR . '/office-of-estate-store';
...
function es_save(string $key, string $val): void
{
    db()->prepare(
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($tab === 'page') {
        ...
        if (empty($errors)) {
            es_save('hero_title', $hero_title); es_save('hero_subtitle', $hero_subtitle);
            es_save('hero_intro', $hero_intro); es_save('meta_description', $meta_desc);
            es_save('is_published', $is_published);
            flash_set('success', 'Page settings saved.');
            redirect(APP_URL . '/office-of-estate-store/settings.php?tab=page');
        }
        ...
        if (!empty($_FILES['head_photo']['name'])) {
            $result = es_upload_photo($_FILES['head_photo']);
            if ($result === false) { $errors[] = 'Photo: invalid file. Allowed: JPG, PNG, GIF, WebP.'; }
            else {
                if ($head_photo) { $old = UPLOAD_DIR . '/office-of-estate-store/' . $head_photo; if (is_file($old)) @unlink($old); }
                $head_photo = $result;
            }
        }
        ...
        if (empty($errors)) {
            es_save('head_name', $head_name); es_save('head_title', $head_title);
            es_save('head_email_1', $head_email_1); es_save('head_email_2', $head_email_2);
            es_save('head_phone', $head_phone); es_save('head_bio', $head_bio);
            es_save('head_photo', $head_photo);
            flash_set('success', 'Head profile saved.');
            redirect(APP_URL . '/office-of-estate-store/settings.php?tab=head');
        }
        ...
        if (empty($errors)) {
            es_save('message_title', $message_title); es_save('message_body', $message_body);
            flash_set('success', 'Message saved.');
            redirect(APP_URL . '/office-of-estate-store/settings.php?tab=message');
        }
    }
...
<div class="d-flex justify-content-between align-items-center mb-4">
    <nav aria-label="breadcrumb">
        <ol class="breadcrumb mb-0">
            ...
            <li class="breadcrumb-item"><a href="<?= APP_URL ?>/office-of-estate-store/index.php">Office of the Estate &amp; Store</a></li>
            <li class="breadcrumb-item active">Settings</li>
        </ol>
    </nav>
    <a href="<?= APP_URL ?>/office-of-estate-store/index.php" class="btn btn-sm btn-light" style="border-radius:8px;">
...
<?php $tabs = ['page'=>['fas fa-cog','Page Settings'],'head'=>['fas fa-building','Head Profile'],'message'=>['fas fa-quote-left','Office Message']];
foreach ($tabs as $key => [$icon, $label]): ?>
    <li class="nav-item">
        <a class="nav-link <?= $active_tab === $key ? 'active' : '' ?>"
           href="<?= APP_URL ?>/office-of-estate-store/settings.php?tab=<?= $key ?>"
...
<form method="POST" enctype="multipart/form-data" novalidate>
    ...
    <div class="card mb-4" style="border-radius:12px;">
        <div class="card-header py-3 px-4"><h6 class="mb-0 fw-semibold"><i class="fas fa-building me-2 text-muted"></i>Head of Estate &amp; Store Profile</h6></div>
        <div class="card-body p-4">
            <div class="mb-4">
                <label class="form-label fw-medium">Profile Photo</label>
                <div class="d-flex align-items-center gap-3 mb-2">
                    <?php if (!empty($s['head_photo'])): ?>
                    <img src="<?= UPLOAD_URL ?>/office-of-estate-store/<?= h($s['head_photo']) ?>" id="head-photo-preview"
                         style="width:80px;height:80px;border-radius:50%;object-fit:cover;border:3px solid #e8eaf0;" alt="Head Profile Photo">
                    <?php else: ?>
                    <div id="head-photo-preview-placeholder" style="width:80px;height:80px;border-radius:50%;background:#f1f5f9;display:flex;align-items:center;justify-content:center;border:2px dashed #cbd5e1;">
                        <i class="fas fa-building" style="color:#94a3b8;font-size:1.6rem;"></i></div>
                    <?php endif; ?>
                    <div>
    ...
    <div class="d-flex gap-2"><button type="submit" class="btn btn-primary" style="border-radius:10px;"><i class="fas fa-save me-1"></i> Save Head Profile</button></div>
</form></div></div>
    ...
    <div class="card mb-4" style="border-radius:12px;">
        ...
        <div class="card-body p-4">
            <div class="mb-3"><label class="form-label fw-medium">Section Title <span class="text-danger">*</span></label>
                ...
                <div class="form-text">E.g. "Message from the Office of the Estate &amp; Store"</div></div>
