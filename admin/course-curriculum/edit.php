...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (empty($errors)) {
        ...
        // Replace marking distributions
        db()->prepare("DELETE FROM cc_mark_distributions WHERE curriculum_id = ?")->execute([$id]);
        if (!empty($valid_dists)) {
            ...
            foreach ($valid_dists as $dist) {
                $dist_stmt->execute([$id, $dist['name'], $dist['marks'], $dist['order']]);
            }
        }
    ...
    // Pre-fill old() from existing row; distributions loaded separately
    save_old([
...
$existing_dists     = cc_get_mark_distributions($id);
                ...
                <div class="card-body p-4">
                    <div class="mb-4">
                        ...
                        <select name="semester" class="form-select">
                            <option value="0" <?= (int)old('semester', $course['semester']) === 0 ? 'selected' : '' ?>>— Not Assigned —</option>
...
// On POST failure re-populate from old(); otherwise load existing DB rows.
var hasOldDists   = <?= json_encode(!empty(old_array('dist_name')), JSON_HEX_TAG) ?>;
var preloadDists  = hasOldDists
    ? <?= json_encode(array_map(null, old_array('dist_name'), old_array('dist_marks'), old_array('dist_order')), JSON_HEX_TAG) ?>
    : <?= json_encode(array_map(function($d) {
          return [$d['distribution_name'], rtrim(rtrim(number_format((float)$d['max_marks'],2),'0'),'.'), $d['sort_order']];
      }, $existing_dists), JSON_HEX_TAG) ?>;
