...
require_access('alumni', 'can_edit');

$id = (int)($_GET['id'] ?? 0);
$st = db()->prepare('SELECT * FROM alumni WHERE id = ?');
$st->execute([$id]);
$alumni = $st->fetch();
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    $status       = in_array($_POST['status'] ?? '', ['pending','approved','rejected']) ? $_POST['status'] : $alumni['status'];
            ...
            if (!in_array($ext, $exts, true)) {
                ...
                if (!in_array($mime, $mimes, true)) {
                    ...
                    if (!move_uploaded_file($f['tmp_name'], $dir . '/' . $new_name)) {
                        ...
                        if ($photo) { $old_path = $dir . '/' . $photo; if (file_exists($old_path)) @unlink($old_path); }
                        $photo = $new_name;
                    }
                }
            }
    ...
    if (!empty($_POST['remove_photo']) && $photo) {
        $old_path = UPLOAD_DIR . '/alumni/' . $photo;
    ...
    if (empty($errors)) {
        db()->prepare(
            'UPDATE alumni SET dept_id=?,name=?,batch=?,company=?,position=?,linkedin_url=?,fb_url=?,photo=?,
    ...
    save_old(compact('dept_id','name','batch','company','position','linkedin_url','fb_url','status','admin_notes','sort_order'));
}
...
<div class="card">
    <div class="card-header py-3 px-4">
        <h6 class="mb-0 fw-semibold"><i class="fas fa-edit me-2 text-muted"></i>Edit Alumni – <?= h($alumni['name']) ?></h6>
    </div>
            ...
            <div class="row g-3">
                <div class="col-md-6">
                    ...
                    <input type="text" name="name" class="form-control" style="border-radius:10px;"
                           value="<?= old('name', h($alumni['name'])) ?>" required maxlength="200">
                </div>
                <div class="col-md-6">
                    ...
                    <select name="dept_id" class="form-select" style="border-radius:10px;">
                        ...
                        <option value="<?= $d['id'] ?>"
                            <?= (old('dept_id', $alumni['dept_id']) == $d['id']) ? 'selected' : '' ?>>
                            <?= h($d['name']) ?>
                        </option>
                        <?php endforeach; ?>
                    </select>
                ...
                <div class="col-md-6">
                    ...
                    <input type="text" name="company" class="form-control" style="border-radius:10px;"
                           value="<?= old('company', h($alumni['company'] ?? '')) ?>" maxlength="200">
                </div>
                <div class="col-md-6">
                    ...
                    <input type="text" name="position" class="form-control" style="border-radius:10px;"
                           value="<?= old('position', h($alumni['position'] ?? '')) ?>" maxlength="200">
                </div>
                <div class="col-md-6">
                    ...
                    <div class="mb-2 d-flex align-items-center gap-3">
                        <img src="<?= UPLOAD_URL ?>/alumni/<?= h($alumni['photo']) ?>"
                             alt="" style="width:70px;height:70px;border-radius:50%;object-fit:cover;border:3px solid #002147;">
                        <div class="form-check">
                            <input class="form-check-input" type="checkbox" name="remove_photo" value="1" id="removePhoto">
                            <label class="form-check-label text-danger small" for="removePhoto">Remove photo</label>
                        </div>
                    </div>
                    <?php endif; ?>
                    ...
                    <div id="photoPreviewWrap" class="mt-2" style="display:none;">
                        <img id="photoPreview" src="" alt="" style="width:80px;height:80px;border-radius:50%;object-fit:cover;border:3px solid #002147;">
                    </div>
                </div>
                <div class="col-md-6">
                    ...
                    <input type="url" name="linkedin_url" class="form-control" style="border-radius:10px;"
                           value="<?= old('linkedin_url', h($alumni['linkedin_url'] ?? '')) ?>" maxlength="500" placeholder="https://linkedin.com/in/…">
                </div>
                <div class="col-md-6">
                    ...
                    <input type="url" name="fb_url" class="form-control" style="border-radius:10px;"
                           value="<?= old('fb_url', h($alumni['fb_url'] ?? '')) ?>" maxlength="500" placeholder="https://facebook.com/…">
                </div>
                <div class="col-md-4">
                ...
                <div class="col-md-4">
                    ...
                    <input type="number" name="sort_order" class="form-control" style="border-radius:10px;"
                           value="<?= old('sort_order', $alumni['sort_order']) ?>" min="0">
                </div>
                <div class="col-md-4 d-flex align-items-end pb-1">
                    <div class="form-check form-switch">
                        <input class="form-check-input" type="checkbox" id="is_active" name="is_active" value="1"
                               <?= old('is_active', $alumni['is_active']) ? 'checked' : '' ?>>
                        <label class="form-check-label" for="is_active">Active</label>
                    </div>
                </div>
                <div class="col-12">
                    <label class="form-label fw-medium">Admin Notes</label>
                    <textarea name="admin_notes" class="form-control" rows="2" style="border-radius:10px;"
                              placeholder="Internal notes (e.g. rejection reason)…"><?= old('admin_notes', h($alumni['admin_notes'] ?? '')) ?></textarea>
                </div>
            </div>
