...
--
--   * `att_sessions` — one row per (offered subject, class date).
--   * `att_records`  — one row per (session, student) with the status.
--
...
CREATE TABLE IF NOT EXISTS `att_sessions` (
  ...
  PRIMARY KEY (`id`),
  UNIQUE KEY `uq_att_subject_date` (`offer_subject_id`,`class_date`),
  KEY `idx_att_sessions_subject` (`offer_subject_id`),
  CONSTRAINT `fk_att_sessions_subject` FOREIGN KEY (`offer_subject_id`)
...
CREATE TABLE IF NOT EXISTS `att_records` (
  `id`         INT(10) UNSIGNED NOT NULL AUTO_INCREMENT,
  `session_id` INT(10) UNSIGNED NOT NULL COMMENT 'FK → att_sessions.id',
  ...
  KEY `idx_att_records_student` (`student_id`),
  CONSTRAINT `fk_att_records_session` FOREIGN KEY (`session_id`)
      REFERENCES `att_sessions` (`id`) ON DELETE CASCADE,
  CONSTRAINT `fk_att_records_student` FOREIGN KEY (`student_id`)
