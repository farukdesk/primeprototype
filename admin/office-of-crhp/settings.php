...
require_access('office-of-crhp', 'can_edit');

$page_title = 'Office of the CRHP – Settings';
$errors     = [];
...
function crhp_upload_photo(array $file): string|false
{
...
function crhp_save(string $key, string $val): void
{
    db()->prepare(
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if ($tab === 'page') {
        ...
        if (empty($errors)) {
            crhp_save('hero_title', $hero_title); crhp_save('hero_subtitle', $hero_subtitle);
            crhp_save('hero_intro', $hero_intro); crhp_save('meta_description', $meta_desc);
            crhp_save('is_published', $is_published);
            flash_set('success', 'Page settings saved.');
            redirect(APP_URL . '/office-of-crhp/settings.php?tab=page');
        }
        ...
        if (!empty($_FILES['head_photo']['name'])) {
            $result = crhp_upload_photo($_FILES['head_photo']);
            if ($result === false) { $errors[] = 'Photo: invalid file. Allowed: JPG, PNG, GIF, WebP.'; }
            else {
                if ($head_photo) { $old = UPLOAD_DIR . '/office-of-crhp/' . $head_photo; if (is_file($old)) @unlink($old); }
                $head_photo = $result;
            }
        }
        ...
        if (empty($errors)) {
            crhp_save('head_name', $head_name); crhp_save('head_title', $head_title);
            crhp_save('head_email_1', $head_email_1); crhp_save('head_email_2', $head_email_2);
            crhp_save('head_phone', $head_phone); crhp_save('head_bio', $head_bio);
            crhp_save('head_photo', $head_photo);
            flash_set('success', 'Head profile saved.');
            redirect(APP_URL . '/office-of-crhp/settings.php?tab=head');
        }
        ...
        if (empty($errors)) {
            crhp_save('message_title', $message_title); crhp_save('message_body', $message_body);
            flash_set('success', 'Message saved.');
            redirect(APP_URL . '/office-of-crhp/settings.php?tab=message');
        }
    }
...
<div class="d-flex justify-content-between align-items-center mb-4">
    <nav aria-label="breadcrumb">
        <ol class="breadcrumb mb-0">
            ...
            <li class="breadcrumb-item"><a href="<?= APP_URL ?>/office-of-crhp/index.php">Office of the CRHP</a></li>
            <li class="breadcrumb-item active">Settings</li>
        </ol>
    </nav>
    <a href="<?= APP_URL ?>/office-of-crhp/index.php" class="btn btn-sm btn-light" style="border-radius:8px;">
...
<?php $tabs = ['page'=>['fas fa-cog','Page Settings'],'head'=>['fas fa-flask','Head Profile'],'message'=>['fas fa-quote-left','Office Message']];
foreach ($tabs as $key => [$icon, $label]): ?>
    <li class="nav-item">
        <a class="nav-link <?= $active_tab === $key ? 'active' : '' ?>"
           href="<?= APP_URL ?>/office-of-crhp/settings.php?tab=<?= $key ?>"
...
<form method="POST" enctype="multipart/form-data" novalidate>
    ...
    <div class="card mb-4" style="border-radius:12px;">
        <div class="card-header py-3 px-4"><h6 class="mb-0 fw-semibold"><i class="fas fa-flask me-2 text-muted"></i>Head of CRHP Profile</h6></div>
        <div class="card-body p-4">
            <div class="mb-4">
                <label class="form-label fw-medium">Profile Photo</label>
                <div class="d-flex align-items-center gap-3 mb-2">
                    <?php if (!empty($s['head_photo'])): ?>
                    <img src="<?= UPLOAD_URL ?>/office-of-crhp/<?= h($s['head_photo']) ?>" id="head-photo-preview"
                    ...
                    <div id="head-photo-preview-placeholder" style="width:80px;height:80px;border-radius:50%;background:#f1f5f9;display:flex;align-items:center;justify-content:center;border:2px dashed #cbd5e1;">
                        <i class="fas fa-flask" style="color:#94a3b8;font-size:1.6rem;"></i></div>
                    <?php endif; ?>
                    <div>
    ...
    <div class="card mb-4" style="border-radius:12px;">
        ...
        <div class="card-body p-4">
            <div class="mb-3"><label class="form-label fw-medium">Section Title <span class="text-danger">*</span></label>
                ...
                <div class="form-text">E.g. "Message from the Office of the CRHP"</div></div>
