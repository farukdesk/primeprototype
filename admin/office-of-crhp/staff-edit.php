...
$id = (int)($_GET['id'] ?? 0);
if ($id <= 0) { redirect(APP_URL . '/office-of-crhp/staff-index.php'); }

$st = null;
try {
    $stmt = db()->prepare('SELECT * FROM crhp_staff WHERE id = ?');
    $stmt->execute([$id]);
    $st = $stmt->fetch();
} catch (Throwable $e) {}
if (!$st) { flash_set('error', 'Staff member not found.'); redirect(APP_URL . '/office-of-crhp/staff-index.php'); }
...
if ($_SERVER['REQUEST_METHOD'] === 'POST') {
    ...
    if (isset($_POST['remove_photo']) && $_POST['remove_photo'] === '1') {
        if ($photo) { $old = UPLOAD_DIR . '/office-of-crhp/' . $photo; if (is_file($old)) @unlink($old); }
        $photo = '';
    }

    if (empty($errors)) {
        db()->prepare(
            'UPDATE crhp_staff SET name=?, title=?, email_1=?, email_2=?, phone=?, photo=?, sort_order=?, is_active=? WHERE id=?'
...
<div class="d-flex justify-content-between align-items-center mb-4">
    <nav aria-label="breadcrumb">
        <ol class="breadcrumb mb-0">
            ...
            <li class="breadcrumb-item"><a href="<?= APP_URL ?>/office-of-crhp/staff-index.php">Staff Directory</a></li>
            <li class="breadcrumb-item active">Edit Staff Member</li>
        </ol>
    </nav>
    <a href="<?= APP_URL ?>/office-of-crhp/staff-index.php" class="btn btn-sm btn-light" style="border-radius:8px;">
...
<form method="POST" enctype="multipart/form-data" novalidate>
        ...
        <div class="card-body p-4">
            ...
            <div class="mb-4">
                ...
                <div class="d-flex align-items-center gap-3 mb-2">
                    ...
                    <img src="<?= UPLOAD_URL ?>/office-of-crhp/<?= h($st['photo']) ?>" id="photo-preview"
    ...
    <div class="d-flex gap-2">
        ...
        <a href="<?= APP_URL ?>/office-of-crhp/staff-index.php" class="btn btn-light" style="border-radius:10px;">Cancel</a>
    </div>
</form>
</div></div>
