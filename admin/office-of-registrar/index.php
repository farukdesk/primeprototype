...
$page_title = 'Office of the Registrar';
...
$staff_count = 0;
try { $staff_count = (int)db()->query('SELECT COUNT(*) FROM reg_staff')->fetchColumn(); } catch (Throwable $e) {}

$is_published = ($s['is_published'] ?? '1') === '1';
$reg_name     = $s['reg_name'] ?? 'Not configured';
$reg_title    = $s['reg_title'] ?? '';
...
<div class="d-flex justify-content-between align-items-center mb-4">
    <nav aria-label="breadcrumb">
        <ol class="breadcrumb mb-0">
            ...
            <li class="breadcrumb-item active">Office of the Registrar</li>
        </ol>
    </nav>
    <a href="<?= $frontend_base ?>/office-of-registrar.php" target="_blank"
...
<div class="alert <?= $is_published ? 'alert-success' : 'alert-warning' ?> d-flex align-items-center gap-2 mb-4" style="border-radius:12px;">
    ...
    <span>Page is currently <strong><?= $is_published ? 'Published' : 'Unpublished' ?></strong>.
        <?php if (!$is_published): ?>
        <a href="<?= APP_URL ?>/office-of-registrar/settings.php" class="alert-link">Enable publishing in settings.</a>
        <?php endif; ?></span>
</div>
            ...
            <div class="card-body p-4">
                <div class="d-flex align-items-start gap-3">
                    ...
                    <div style="width:56px;height:56px;border-radius:50%;background:linear-gradient(135deg,#002147,#1a4faf);display:flex;align-items:center;justify-content:center;flex-shrink:0;">
                        <i class="fas fa-stamp" style="color:#fff;font-size:1.3rem;"></i>
                    </div>
                    <?php endif; ?>
                    <div class="flex-grow-1">
                        <h6 class="mb-1 fw-semibold" style="color:#002147;"><i class="fas fa-stamp me-2 text-muted" style="font-size:.85rem;"></i>Registrar</h6>
                        <div class="fw-medium"><?= h($reg_name) ?></div>
                        <div style="font-size:.8rem;color:#64748b;"><?= h($reg_title) ?></div>
                    </div>
                </div>
                <hr style="border-color:#f1f5f9;margin:16px 0 12px;">
                <?php if (can_access('office-of-registrar', 'can_edit')): ?>
                <a href="<?= APP_URL ?>/office-of-registrar/settings.php?tab=registrar" class="btn btn-sm btn-primary" style="border-radius:8px;font-size:.8rem;">
                    <i class="fas fa-edit me-1"></i> Edit Registrar Profile
                </a>
                <?php endif; ?>
            </div>
        </div>
    ...
    <div class="col-xl-6 col-lg-6">
        <div class="card h-100" style="border-top:4px solid #7c3aed;border-radius:12px;">
            <div class="card-body p-4">
                ...
                <hr style="border-color:#f1f5f9;margin:16px 0 12px;">
                <a href="<?= APP_URL ?>/office-of-registrar/staff-index.php" class="btn btn-sm btn-outline-secondary me-2" style="border-radius:8px;font-size:.8rem;">
                    <i class="fas fa-list me-1"></i> Manage Staff
                </a>
                <?php if (can_access('office-of-registrar', 'can_create')): ?>
                <a href="<?= APP_URL ?>/office-of-registrar/staff-create.php" class="btn btn-sm btn-outline-secondary" style="border-radius:8px;font-size:.8rem;">
        ...
        <div class="card h-100" style="border-top:4px solid #D21034;border-radius:12px;">
            <div class="card-body p-4">
                <div class="d-flex align-items-start gap-3">
                    ...
                    <div class="flex-grow-1">
                        <h6 class="mb-1 fw-semibold" style="color:#D21034;">Registrar's Message</h6>
                        <div style="font-size:.82rem;color:#64748b;"><?= h($s['message_title'] ?? 'Message from the Registrar') ?></div>
                ...
                <hr style="border-color:#f1f5f9;margin:16px 0 12px;">
                <?php if (can_access('office-of-registrar', 'can_edit')): ?>
                <a href="<?= APP_URL ?>/office-of-registrar/settings.php?tab=message" class="btn btn-sm btn-outline-danger" style="border-radius:8px;font-size:.8rem;">
            ...
            <div class="card-body p-4">
                <div class="d-flex align-items-start gap-3">
                    ...
                    <div class="flex-grow-1">
                        ...
                        <div style="font-size:.82rem;color:#64748b;"><?= h($s['hero_title'] ?? 'Office of the Registrar') ?></div>
                        <div class="mt-2">
                ...
                <hr style="border-color:#f1f5f9;margin:16px 0 12px;">
                <?php if (can_access('office-of-registrar', 'can_edit')): ?>
                <a href="<?= APP_URL ?>/office-of-registrar/settings.php?tab=page" class="btn btn-sm btn-outline-success" style="border-radius:8px;font-size:.8rem;">
